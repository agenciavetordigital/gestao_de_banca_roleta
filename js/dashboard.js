...
async function loadData() {
  ...
  // As duas buscas abaixo agora são feitas em paralelo, muito mais rápido
...
function renderAll() {
  ...
  // Injete o HTML do dashboard de uma vez
  ...
  // Chame as funções para preencher cada parte
  renderMetrics();
  renderSettings();
  renderDailyGoal();
  renderBetForm();
...
function renderDailyGoal() {
    ...
    let content = `
        ...
        <div id="daily-goal-content" class="space-y-4">
            <div class="flex justify-between text-sm"><span class="text-gray-400">Lucro do Ciclo:</span><span id="today-profit" class="font-semibold">${formatCurrency(cycleProfit)}</span></div>
            <div class="flex justify-between text-sm"><span class="text-gray-400">Meta do Ciclo:</span><span id="today-target" class="font-semibold">${formatCurrency(targetProfit)}</span></div>
...
function setupEvents() {
    ...
    // Preenchendo os eventos que foram omitidos
    document.getElementById("save-bankroll-btn")?.addEventListener("click", async () => {
    ...
    document.getElementById("add-bet-form")?.addEventListener("submit", async (e) => {
        ...
        else { jogadas.unshift(data); document.getElementById('add-bet-form').reset(); renderAll(); }
    });
    ...
    // O addDeleteEventListeners precisa ser chamado dentro de renderBets
}
        });
    </script>
</body>
